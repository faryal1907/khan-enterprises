...
import { collectReceivable, getPaymentAccounts } from "@/lib/api/accounting";
...
export function CollectReceivableModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await collectReceivable(customer.customerPhone, {
        amount,
        paymentMethod,
      ...
      if (result.isAdvance) {
        toast.info(`Advance credit: Rs. ${result.advanceAmount.toLocaleString()} held for future orders.`);
      }
      onSuccess();
      onClose();
    } catch (err: any) {
  ...
  const isOverpayment = amount > customer.totalOutstanding;
    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="text-xs mt-0.5" style={{ color: theme.text.secondary }}>
            {customer.customerPhone}
          </div>
          <div className="text-xs mt-1 font-semibold text-red-600">
            Outstanding: Rs. {customer.totalOutstanding.toLocaleString()}
          </div>
        </div>
        ...
        <div className="space-y-4 mb-6">
          <div>
            ...
            {isOverpayment && (
              <p className="text-xs mt-1 font-medium text-amber-600">
                Amount exceeds outstanding balance — Rs.{" "}
                {(amount - customer.totalOutstanding).toLocaleString()} will be held as advance credit.
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              className="w-full border rounded-md p-2 outline-none bg-transparent text-sm"
              style={{ color: theme.text.primary, borderColor: theme.borders?.medium || "#e5e7eb" }}
