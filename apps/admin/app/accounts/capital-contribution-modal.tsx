...
import { recordCapitalContribution, getAccounts } from "@/lib/api/accounting";
import { AsyncButton } from "@/components/async-button";
import { toast } from "sonner";

type CapitalContributionModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
};

export function CapitalContributionModal({ isOpen, onClose, onSuccess }: CapitalContributionModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await recordCapitalContribution({
      ...
      toast.error(err.message || "Failed to record capital contribution");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g., Salary, Freelancing, Personal Savings"
              className="w-full px-3 py-2 border rounded-md text-sm text-black"
            />
          </div>
        </div>
