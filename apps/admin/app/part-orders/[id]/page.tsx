...
import { getPartOrderById, updatePartOrderStatus, cancelPartOrder, downloadInvoice, markPartOrderAsPickedByCustomer, verifyPartPayment, recordPartOrderPayment, completePartOrderDetails } from "@/lib/api/orders";
...
export default function PartOrderDetailPage() {
  ...
  const handleEditCustomerInfo = async () => {
    try {
      setActionLoading(true);
      await completePartOrderDetails(orderId, customerData);
      const updatedOrder = await getPartOrderById(orderId);
      setOrder(updatedOrder);
  ...
  const anyReversedPayment = order?.transactions?.some((tx: any) => tx.isReversed);

  const renderTxStatus = (tx: any) => {
    if (tx.isReversed) {
      return (
